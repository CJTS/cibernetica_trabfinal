...

{
  ...
  case ALL:
    printf("ALL %s : %s\n", plnode->atom->item, plnode->atom->next->item);
    print_indent(indent);
    printf("(   ");
...

{
  ...
  switch (n->connective) {
  case ALL:
    printf("ALL x%d (%s): %s\n", n->var, n->var_name, gtype_names[n->var_type]);
    print_indent(indent);
    printf("(   ");
...

{
  ...
  printf("\n\n----------------Operator %s, translated form, step "
         "1--------------\n",
         o->name);

  for (i = 0; i < o->num_vars; i++) {
    printf("\nx%d (%s) of type %s, removed ? %s", i, o->var_names[i],
           gtype_names[o->var_types[i]], o->removed[i] ? "YES" : "NO");
  }
  ...
  for (e = o->effects; e; e = e->next) {
    ...
    for (i = 0; i < e->num_vars; i++) {
      printf("\nx%d (%s) of type %s", o->num_vars + i, e->var_names[i],
             gtype_names[e->var_types[i]]);
    }
    printf("\nConditions\n");
...

{
  ...
  printf("\n\n%d vars removed from original operator:", o->num_removed_vars);
...

{
  ...
  for (e = o->effects; e; e = e->next) {
    ...
    for (i = 0; i < e->num_vars; i++) {
      printf("\nx%d of type %s", o->operator->num_vars + i,
             gtype_names[e->var_types[i]]);
    }
    printf("\nConditions\n");
    print_Wff(e->conditions, 0);
...
char **get_plan(void) {
  ...
  char **plan = (char **)calloc(gnum_plan_ops + 1, sizeof(char *));

  for (i = 0; i < gnum_plan_ops; i++) {
    plan[i] = get_op_name(gplan_ops[i]);
  }
  plan[i] = NULL;
